		...
		<view class="list-box" @click="toDetail(item.id)" v-for="item in inspectionList" :key="item.id">
			...
			<view class="location grey-color">
				<view class="overflow-ellipsis">
					...
					<text v-for="(item1, i) in item[classifyMap[item.pointType]]" :key="i">
						{{item1.place}}{{i !== item[classifyMap[item.pointType]].length - 1 ? '、' : ''}}
					</text>
				</view>
			</view>
...
<script>
	export default {
		...
		data() {
			return {
				classifyMap:['allPointList', 'allPointList', 'allGovernmentAgencyList']
			}
		},
		methods: {
			toDetail(id) {
				uni.navigateTo({
					url: `/pages/garbageClassification/garbageSorting/modules/inspection-detail/inspection-detail?id=${id}`,
